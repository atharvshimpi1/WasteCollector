...
import my.android.wastecollector.model.mUserToMap
...
class AuthViewmodel:ViewModel() {
    ...
    fun completeUserProfile(mUser: MUser,userId:String) {
        ...
        viewModelScope.launch {
            try {
                firestore.collection("wasteUsers").document(userId)
                    .set(
                        mUserToMap(mUser)
                    ).await()
    ...
    private suspend fun getUserProfile(userId: String): MUser? {
        return try {
            val document = firestore.collection("wasteUsers").document(userId).get().await()
            if (document.exists()) {
